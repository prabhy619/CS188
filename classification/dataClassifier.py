...
def putNeighboursinSet(st,datum,i,j,mode):
    
    if mode=='all':
        VALIDITY=range(DIGIT_DATUM_HEIGHT)
    elif mode=='upper':
        VALIDITY=range(DIGIT_DATUM_HEIGHT/2,DIGIT_DATUM_HEIGHT)
    elif mode=='lower':
        VALIDITY=range(DIGIT_DATUM_HEIGHT/2)
    
    if i>=DIGIT_DATUM_WIDTH or i<0 or (j not in VALIDITY) :
        return
    if datum.getPixel(i,j)==0 and (i,j) not in st:
        st.add((i,j))
        putNeighboursinSet(st,datum,i+1,j,mode)
        putNeighboursinSet(st,datum,i,j+1,mode)
        putNeighboursinSet(st,datum,i,j-1,mode)
        putNeighboursinSet(st,datum,i-1,j,mode)
        
        

def getLoops(datum,mode):
    a=datum.getPixels()
    features=util.Counter()
    counter=0
    nun=set()
    ...
    for i in range(DIGIT_DATUM_WIDTH):
        for j in VALIDITY:
            if datum.getPixel(i,j)==0 and (i,j) not in nun:
                putNeighboursinSet(nun,datum,i,j,mode)
                counter+=1
#        pdb.set_trace()
    return counter
...
def getLimits(datum):
    ...
    for i in reversed(range(DIGIT_DATUM_WIDTH)):
        if uuu!=-1:
            break
        for j in reversed(range(DIGIT_DATUM_HEIGHT)):
            if datum.getPixel(i,j)>0:
                uuu=i
                break
...
def getNeigboursDirection(datum):
    a=datum.getPixels()
    left,down,counter=0,0,0
    for x in range(DIGIT_DATUM_HEIGHT):
        for y in range(DIGIT_DATUM_WIDTH):
            if datum.getPixel(x,y)>0:
                counter+=1
                LG=x
                while LG>=0 and datum.getPixel(LG,y)>0:
                    left+=1
                    LG=LG-1
                
                LG=y
                while LG>=0 and datum.getPixel(x,LG)>0:
                    down+=1
                    LG=LG-1
    return (1.0*left)/(1.0*counter),(1.0*down)/(1.0*counter)

def getVerticalIntercepts(datum):
    score=[]
    for x in range(DIGIT_DATUM_WIDTH):
        found=False
        changed=0
        ...
        for y in range(DIGIT_DATUM_HEIGHT):
            ...
            if status!=changed:
                ...
                changed=status
        if counter!=0:
            score.append(counter/2)
    return (1.0*sum(score))/(1.0*len(score));
            
def getContigous(ls,num):
    tmp=[]
    for i in ls:
        if i==0:
            tmp.append(0)
...
def getVerticalLines(datum):
    counter=0
    for x in range(DIGIT_DATUM_WIDTH):
        ll=[]
        for y in range(DIGIT_DATUM_HEIGHT):
            ll.append(datum.getPixel(x,y))
        counter+=getContigous(ll,14)
    return counter

def getHorizontalLines(datum):
    counter=0
    for x in range(DIGIT_DATUM_WIDTH):
        ll=[]
        for y in range(DIGIT_DATUM_HEIGHT):
            ll.append(datum.getPixel(y,x))
        counter+=getContigous(ll,9)
    return counter
...
def enhancedFeatureExtractorDigit(datum):
    ...
    "*** YOUR CODE HERE ***"
    #features=util.Counter()
    features['loop']=getLoops(datum,'all')
    features['ul']=getLoops(datum,'upper')-1#just to remove whitespace surrounding
    features['ll']=getLoops(datum,'lower')-1#just to remove whitespace surrounding
    features['extense']=getLimits(datum)
    features['leftN'],features['downN']=getNeigboursDirection(datum)
    features['vIntercepts']=getVerticalIntercepts(datum)
    features['vLines']=getVerticalLines(datum)
    features['hLines']=getHorizontalLines(datum)
    print(datum)
    print(features['loop'],features['ul'],features['ll'],features['extense'],features['leftN'],features['downN'],features['vIntercepts'],features['hLines'],features['vLines'])
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    """
    "*** YOUR CODE HERE ***"
    nowFood=state.getNumFood()
    state=state.generateSuccessor(0,action)
    features=util.Counter()
    nd=2
    for ghost in range(state.getNumAgents()-1):
        dis=util.manhattanDistance(state.getGhostPosition(ghost+1),state.getPacmanPosition())
        if dis<util.manhattanDistance(state.getGhostPosition(nd),state.getPacmanPosition()):
            nd=ghost+1
    features['moves']=len(state.getLegalActions())-1
    features['nghPos']=util.manhattanDistance(state.getGhostPosition(nd),state.getPacmanPosition())
    features['ngState']=1 if state.getGhostState(nd).scaredTimer>0 else 0
    features['vghPos']=util.manhattanDistance(state.getGhostPosition(3-nd),state.getPacmanPosition())
    features['foodCount']=nowFood-state.getNumFood()
    if nowFood-state.getNumFood()==1:
        features['nearestFood']=-90
    else:
        features['nearestFood']=getNearestFood(state)
    if action=='Stop':
        features['nearestFood']=features['nearestFood']+1
    #print features
    return features

#i did define a bfs algorithm to get the nearest food but not gonna do it again oon account of it not helping my heurestic, however if it works I think i used a bfs  algo to make it efficient in project 2
def getNearestFood(state):
    ...
    for x in range(state.getWalls().width):
        for j in range(state.getWalls().height):
            if allFood[x][j]:
                if dis>util.manhattanDistance((x,j),pacPos):
                    dis=util.manhattanDistance((x,j),pacPos)
    return dis
